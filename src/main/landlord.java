...
public class landlord {
    ...
    private void addRoom(Scanner sc) {
        ...
        System.out.println("✅ Room added successfully and is now available for tenants.");
    }

    private void viewMyRooms() {
    System.out.println("\n=== MY ROOMS ===");
    String sql = "SELECT r_id, r_name, r_type, r_price, r_desc, r_status, r_loc "
    ...
    if (rooms.isEmpty()) {
        ...
        System.out.printf("%-5s %-15s %-10s %-10s %-30s %-12s %-15s%n",
                "ID", "Name", "Type", "Price", "Description", "Status", "Location");
        for (Map<String, Object> r : rooms) {
            System.out.printf("%-5s %-15s %-10s %-10s %-30s %-12s %-15s%n",
                    r.get("r_id"), r.get("r_name"), r.get("r_type"),
                    r.get("r_price"), r.get("r_desc"),
                    r.get("r_status"), r.get("r_loc"));
        }
        System.out.println("You have " + rooms.size() + " rooms listed.");
    }
}
    ...
    private void viewReservations(Scanner sc) {
        ...
        System.out.printf("%-5s %-15s %-12s %-12s %-10s %-10s%n", "ID", "Tenant", "Room", "Move-in", "Duration", "Status");
        for (Map<String, Object> r : res) {
            System.out.printf("%-5s %-15s %-12s %-12s %-10s %-10s%n",
                    r.get("res_id"), r.get("tenant_name"), r.get("r_name"),
